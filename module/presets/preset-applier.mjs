...
export class PresetApplier {
  ...
  static async _applySettings(preset) {
    ...
    // Appliquer les couleurs DSN du preset si définies
    if (preset.dsnColors) {
      const colorMap = {
        dsnColorStressFg: preset.dsnColors.stressFg,
        dsnColorStressBg: preset.dsnColors.stressBg,
        dsnColorGearFg:   preset.dsnColors.gearFg,
        dsnColorGearBg:   preset.dsnColors.gearBg,
      };
      for (const [key, value] of Object.entries(colorMap)) {
        if (value) {
          try { await game.settings.set("yzegenerique", key, value); }
          catch(e) { console.warn(`YZE Preset | DSN color "${key}":`, e); }
        }
      }
    }
    if (preset.dsnLabels) {
      const labelMap = {
        dsnLabelBane:       preset.dsnLabels.bane,
        dsnLabelSuccess:    preset.dsnLabels.success,
        dsnLabelStressBane: preset.dsnLabels.stressBane,
      };
      for (const [key, value] of Object.entries(labelMap)) {
        if (value !== undefined) {
          try { await game.settings.set("yzegenerique", key, value); }
          catch(e) { console.warn(`YZE Preset | DSN label "${key}":`, e); }
        }
      }
    }
    if (preset.titleFont) {
      try { await game.settings.set("yzegenerique", "titleFont", preset.titleFont); }
      catch(e) { console.warn("YZE Preset | titleFont:", e); }
    }
    if (preset.themeColors) {
      try {
        if (preset.themeColors.accent) await game.settings.set("yzegenerique", "themeColorAccent", preset.themeColors.accent);
        if (preset.themeColors.bg)     await game.settings.set("yzegenerique", "themeColorBg",     preset.themeColors.bg);
      } catch(e) { console.warn("YZE Preset | themeColors:", e); }
    }
  }
