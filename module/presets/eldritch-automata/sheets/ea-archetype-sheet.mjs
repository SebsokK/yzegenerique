...
export class EaArchetypeSheet extends YZESheetMixin(HandlebarsApplicationMixin(foundry.applications.sheets.ItemSheetV2)) {
  ...
  async _prepareContext(options) {
    ...
    context.enriched.breakdownText = await foundry.applications.ux.TextEditor.implementation.enrichHTML(this.item._source.system.breakdownText || '', { async: true });
    context.enriched.strandTalentDescription = await foundry.applications.ux.TextEditor.implementation.enrichHTML(this.item._source.system.strandTalentDescription || '', { async: true });
    context.enriched.hopeTalentDescription = await foundry.applications.ux.TextEditor.implementation.enrichHTML(this.item._source.system.hopeTalentDescription || '', { async: true });
    context.enriched.berserkText = await foundry.applications.ux.TextEditor.implementation.enrichHTML(this.item._source.system.berserkText || '', { async: true });
    return context;
  }
