...
export class PanicHandler {
  ...
  static async trigger(actor, rollData) {
    ...
    if (!table) {
    await PanicHandler._sendFallbackToChat(actor, dieRoll, dieResult, stressLevel, total, tableName);
      return;
    }
    ...
    await PanicHandler._sendPanicToChat(actor, dieRoll, dieResult, stressLevel, total, entryText, table.name);
  }
   ...
   */
  static async _sendPanicToChat(actor, dieRoll, dieResult, stressLevel, total, entryText, tableName) {
    const content = `
    ...
    const msgData = {
      ...
      content,
      rolls:   [dieRoll],   // ← Dice So Nice picks this up
    };
   ...
   */
  static async _sendFallbackToChat(actor, dieRoll, dieResult, stressLevel, total, tableName) {
    const content = `
      <div class="yze-panic-roll">
